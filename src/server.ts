            ...
            removeMaybesFromArray<ErrorRequestHandler | RequestHandler>(
                ...
                async function (
                    ...
                    try {
                        ...
                        if (r.render) {
                            const data = await r.function.apply(
                                controllerClass,
                                args
                            );
                            if (res.headersSent)
                            ...
                            const value = await r.function.apply(
                                controllerClass,
                                args
                            );
...
export class Server {
    ...
    addController<T extends new () => any>(...controller: T[]) {
        for (const c of controller) {
            ...
            if (!router) continue;
            if (router[0].length < 1 || router[0] === '/')
                this.app.use(router[1]);
    ...
    logValue(severity: ServerTypes['logSeverites'], ...values: any[]) {
        ...
        const prefix = constants.server.logSeverityPrefixes[
            severity
        ].replaceAll('{time}', new Date().toUTCString());
